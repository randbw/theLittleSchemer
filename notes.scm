...
;; entry is pair of lists which have same length and the first is a set
((list "apple" "banana" "pear") (list "turkey" "turkey" "chicken"))

(define build-entry
  (lambda (set values)
    (build (set values))))
;; simplified
(define build-entry build)
...
(define lookup-in-entry-help
  (lambda (name names values entry-f)
    (cond
     ...
     (else
      (lookup-in-entry-help name (cdr names) (cdr value) entry-f)))))

;; a table (aka environment) is a list of entries

(define table-de-rand
  (((frog possum) (legs tail)) ((chicken derp) (kfc wut))))
